...
| Payments        | PayPal (webhooks), COD (admin approval), Crypto (Ethereum Sepolia smart contract) |
...
### Crypto (Ethereum Sepolia Smart Contract) — `CRYPTO_PAYMENT_ENABLED=true`
...
3. Client calls `payOrder(proof)` on `NgooPayment.sol` (Ethereum Sepolia, chain 11155111)
